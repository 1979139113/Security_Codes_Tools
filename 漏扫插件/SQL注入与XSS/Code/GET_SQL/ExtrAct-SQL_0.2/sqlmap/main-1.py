...
FangSongDa = document.styles.add_style('FangSongDa', 1)
# 设置字体尺寸
FangSongDa.font.size = Pt(16)
# 设置字体颜色
FangSongDa.font.color.rgb = RGBColor(0,0,0)
# 黑色
#UserStyle1.font.color.rgb = RGBColor(0xff, 0xde, 0x00)
# 居中文本
FangSongDa.font.name = '仿宋'
FangSongDa._element.rPr.rFonts.set(qn('w:eastAsia'), '仿宋')

FangSongZh = document.styles.add_style('FangSongZh', 1)
# 设置字体尺寸
FangSongZh.font.size = Pt(9)
# 设置字体颜色
FangSongZh.font.color.rgb = RGBColor(178,34,34)
# 红色
#UserStyle1.font.color.rgb = RGBColor(0xff, 0xde, 0x00)
# 居中文本
FangSongZh.font.name = '仿宋'
FangSongZh._element.rPr.rFonts.set(qn('w:eastAsia'), '仿宋')

FangSongZhi = document.styles.add_style('FangSongZhi', 1)
# 设置字体尺寸
FangSongZhi.font.size = Pt(10)
# 设置字体颜色
FangSongZhi.font.color.rgb = RGBColor(0,0,0)
# 黑色
#UserStyle1.font.color.rgb = RGBColor(0xff, 0xde, 0x00)
# 居中文本
FangSongZhi.font.name = '仿宋'
FangSongZhi._element.rPr.rFonts.set(qn('w:eastAsia'), '仿宋')


FangSongXi = document.styles.add_style('FangSongXi', 1)
# 设置字体尺寸
FangSongXi.font.size = Pt(6.5)
# 设置字体颜色
FangSongXi.font.color.rgb = RGBColor(0,255,0)
# 绿色
#UserStyle1.font.color.rgb = RGBColor(0xff, 0xde, 0x00)
# 居中文本
FangSongXi.font.name = '仿宋'
FangSongXi._element.rPr.rFonts.set(qn('w:eastAsia'), '仿宋')
...
def Common_Database(common,content,url):
    ...
    document.add_paragraph('即当应用程序使用输入内容来构造动态SQL语句以访问数据库时，如果对输入的参数没有进行严格的过滤或者过滤不完整将会导致SQL注入攻击的产生。', 'FangSongDa')
    ...
    try:
        ...
        document.add_paragraph(dbs.replace('the back-end DBMS is ', '数据库类型 : ').replace('web server operating system: ',
                                                                       '服务器版本 : ').replace(
            'web application technology: ', '服务器语言 : ').replace('back-end DBMS: ', '数据库版本 : '),'FangSongZhi')
        ...
        infos = info.scan_seo()
        for k,v in infos:
            document.add_paragraph(k+'  '+v,'FangSongZhi')

        document.add_page_break()

        document.add_paragraph('网站漏洞复现', 'Subtitle')
        document.add_paragraph('执行命令','FangSongDa')
        document.add_paragraph(common,'FangSongZh')
        document.add_paragraph('返回结果','FangSongDa')
        document.add_paragraph(content, 'FangSongZh')
        document.add_paragraph('敏感数据','FangSongDa')
        document.add_paragraph(inj.replace('Parameter: ', '注入参数(方式) : ').replace('Type: ', '注入方式 : ').replace('Title: ',
                                                                                             '注入标题 : ').replace(
            'Payload: ', '注入攻击 : '),'FangSongZh')
        document.add_paragraph(dbs.replace('the back-end DBMS is ', '数据库类型 : ').replace('web server operating system: ',
                                                                       '服务器版本 : ').replace(
            'web application technology: ', '服务器语言 : ').replace('back-end DBMS: ', '数据库版本 : '),'FangSongZh')
...
def Get_Column(content):
    try:
        ...
        document.add_paragraph(content, 'FangSongZh')
        document.add_paragraph('敏感数据', 'FangSongDa')
        document.add_paragraph(all_column, 'FangSongZh')
        document.add_page_break()
...
def main(url):
    ...
    if Check_Inj_Result(result_1):
        # 优先写入通用结果
        Common_Database(common_1,result_1,domain)
    ...
    if Check_Inj_Result(result_1) == 'ACCESS':
        ...
        # 尝试写入表名
        document.add_paragraph('执行命令', 'FangSongDa')
        document.add_paragraph(common_2,'FangSongZh')
        Get_Column(result_2)
        Scan_Status = 1
        # access到此为止就ok了
    ...
    if Check_Inj_Result(result_4):
        Common_Database(common_4,result_4,domain)
...
if __name__ == '__main__':
    main('http://127.0.0.1/sqli/Less-1/?id=1')
