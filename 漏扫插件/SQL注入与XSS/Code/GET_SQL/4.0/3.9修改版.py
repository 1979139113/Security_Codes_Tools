    ...
    if link == None:
        ...
        if id_links:
            for id_link in id_links:
                if level == 0:
                    dix = scan_level_0(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 1:
                    dix = scan_level_1(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 2:
                    dix = scan_level_2(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 3:
                    dix = scan_level_3(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 4:
                    dix = scan_level_4(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 5:
                    dix = scan_level_5(id_link.strip())
                    if dix == 'INJ':
                        return
                if level == 6:
...
if __name__ == '__main__':
    ...
    print('''
        ...
        Description:
            Langzi_SQL_INJECTION v4.0版本是一款批量检测SQL注入漏洞的自动化工具
