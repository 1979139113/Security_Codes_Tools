...
import re
from urllib.parse import  urlparse,parse_qsl,urlunsplit
...
def assign(service, arg):
    ...
    # 分析链接，看有没有查询参数
    r = urlparse(arg)
    pairs = parse_qsl(r.query)
    # 如果参数过多，超过6个，效率起见，放弃此链接
    if urlparse(arg).query.find('=') == -1 or len(pairs) > 6:
        return
    ...
    for i in range(loop):
        for file in files:
            ...
            if url not in filter:
                filter[url] = True
                # 请求URL
                code, head, res, _, new_url = curl.get(url)
                print('[%03d] %s', code, url)
                # 开始查询是否包含签名内容, 如<?php <%
                for w in flags:
                    if re.search(w, res):
...
def audit(arg):
    ...
    r = urlparse(url)
    # 取出?号前面的地址
    action = urlunsplit((r.scheme, r.netloc, r.path, '', ''))
    # 取出参数的key
    pairs = parse_qsl(r.query)
    ...
    code, head, normal_res, _, _ = curl.get(action)
