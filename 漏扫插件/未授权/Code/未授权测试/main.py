...
def brute(ip):
    print 'IS ON RUNNING...'
    # print'Redis数据库未授权访问漏洞 : ' + str(ip) + ':' + str(6379)
    try:
        s = socket.socket()
    ...
    # print'Mongodb数据库未授权访问漏洞 : ' + str(ip) + ':27017'
    try:
        conn = pymongo.MongoClient(str(ip), 27017)
        if conn.database_names():
            success('Mongodb数据库未授权访问漏洞 : ' + str(ip) + ':27017')
        return ''
    except Exception,e:
        pass
    ...
    try:
        ...
        if conn.database_names():
            success('Mongodb数据库未授权访问漏洞 : ' + str(ip) + ':27018')
        return ''
    except Exception,e:
        pass

    # print'ZooKeeper未授权访问漏洞 : ' + str(ip) + ':2181'
    try:
        s = socket.socket()
        ...
        if "zookeeper.version" in result:
            success('ZooKeeper未授权访问漏洞 : ' + str(ip) + ':2181')
            return ''
    except Exception, e:
        pass
    finally:
    ...
    # print'Elasticsearch未授权访问漏洞 : ' + str(ip) + ':9200'
    try:
        ...
        if resp.status == 200:
            success('Elasticsearch未授权访问漏洞 : ' + str(ip) + ':9200')
            return ''
    except Exception, e:
        pass
    finally:
    ...
    # print'Memcache未授权访问漏洞 : ' + str(ip) + ':11211'
    try:
        s =socket.socket()
        ...
        if "STAT version" in result:
            success('Memcache未授权访问漏洞 : ' + str(ip) + ':11211')
            return ''
    except Exception, e:
        pass
    finally:
    ...
    # print'CouchDB未授权访问漏洞 : ' + 'http://' + str(ip) + '/_config'
    try:

        rr = requests.get(url=str('http://' + str(ip) + '/_config'), timeout=timeout)
        if "couch" in rr.content:
            success('CouchDB未授权访问漏洞 : ' + str(rr.url))
            return ''
    except Exception, e:
        pass

    # print'Jenkins未授权访问漏洞 : ' +'http://' + str(ip) + '/manage '
    try:
        ...
        for r_r in r_:
            try:
                ...
                if 'arbitrary' in rxr.content:
                    success('Jenkins未授权访问漏洞 : ' + str(r_r))
                    # # log(res)
                    return ''
            except Exception, e:
                pass
    ...
    # print'WebServer任意文件读取漏洞 : ' + str(ip)+':80'
    try:
        ...
        if 'root:' in data and 'nobody:' in data:
            success('WebServer任意文件读取漏洞 : ' + str(ip)+':80')
            # # log(res)
            return ''
    except Exception, e:
        pass
    ...
    try:
        ...
        s.close()
        if 'root:' in data and 'nobody:' in data:
            success('WebServer任意文件读取漏洞 : ' + str(ip)+':443')
            # # log(res)
            return ''
    except Exception, e:
        pass
    ...
    try:
        ...
        s.close()
        if 'root:' in data and 'nobody:' in data:
            success('WebServer任意文件读取漏洞 : ' + str(ip)+':8080')
            return ''
    except Exception, e:
        pass
    finally:
    ...
    # print'Fast-Cgi文件读取漏洞 : ' + str(ip) + ':9000'
    try:
        ...
        try:
            ret = s.recv(1024)
            if ret.find(':root:') > 0:
                success('Fast-Cgi文件读取漏洞 : ' + str(ip) + ':9000')
                # # log(res)
                return ''
        except Exception, e:
            pass
    ...
    # print'HTTP.sys远程代码执行漏洞 : ' + str(ip) + ':80'
    try:
    ...
    # print'Resin viewfile远程文件读取漏洞 : ' + str(ip)
    headers = {
