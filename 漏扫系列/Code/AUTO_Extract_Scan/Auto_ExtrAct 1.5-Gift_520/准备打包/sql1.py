...
def Run_Sqlmap(common):
    ...
    try:
       # print('Scan : '+common.split(' ')[3])
        ...
        print('[{}] Scan : '.format(str(datetime.datetime.now()).split('.')[0])+common.replace('--random-agent --dbs  --current-user --current-db --is-db --batch','').replace('sqlmap\\',''))
...
def Common_Database(document,common,content,url,sqlurl):
    ...
    try:
        ...
        document.add_paragraph('SQLMAP高版本的boundaries.xml没有了针对模糊查询(%)的测试，并且必须手动设置json的某个参数为*才能对这个参数进行注入，否则payload直接跟在json后导致无法注入','DaHei')
...
def test_start():
    with connect_mysql() as conn:
        sql1 = 'select count(id) from sec_get_links where sql_get=0'
        conn.execute(sql1)
        res = conn.fetchone()
        if res == None:
