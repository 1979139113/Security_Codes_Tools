...
def get_xss(sem,level=1):
    time.sleep(random.randint(1,15))
    while 1:
        sem.acquire()
        with connect_mysql() as conn:
            sql1 = 'select links from sec_get_links where vlun_get=0 limit 1'
            sql2 = 'update sec_get_links set vlun_get=1 where vlun_get=0 limit 1'
            conn.execute(sql1)
            res = conn.fetchone()
            if res == None:
                sem.release()
            ...
            sem.release()
            # conn.execute(sql1)
            # Url_Links = eval(conn.fetchone()[0])
            # conn.execute(sql2)
            # sem.release()
        if Url_Links:
...
def test_start():
    with connect_mysql() as conn:
        sql1 = 'select count(id) from sec_get_links where vlun_get=0'
        conn.execute(sql1)
        res = conn.fetchone()
        if res == None:
...
if __name__ == '__main__':
    ...
    if t_start:
        ...
        for i in range(thread_s):
            pool.apply_async(func=get_xss, args=(sem,levels))
        pool.close()
        pool.join()
        print('数据库中URL跳转漏洞扫描完毕~')
        os.system('pause')
