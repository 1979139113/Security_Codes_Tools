...
class Check_Env:
    def __init__(self):
        print('\n')
        print('******配置文件相关信息***'.center(30))
        print(f'账号:{user}'.center(30))
        print(f'密码:{passwd}'.center(30))
        print(f'线程:{thread_s}'.center(30))
        print(f'端口号:{port}'.center(30))
        print(f'检测环境:{check_}'.center(30))
        print(f'扫描文件:{start_}'.center(30))
        print('******配置文件相关信息******'.center(30))
        self.envinu = 1
        print('\n')
    ...
    def check_mod(self):
        try:
            ...
            print('[*] 开始扫描功能模块......')
            if os.path.exists('ExtrBae.dll') and os.path.exists('ExtrBas.dll') and os.path.exists('ExtrSql.dll') and os.path.exists('ExtrUrs.dll') and os.path.exists('ExtrXss.dll'):
                print('[+] 扫描功能模块齐全')
            else:
                print('[-] 扫描功能模块不齐全')
                self.envinu = 0
    ...
    def Get_Result(self):
        ...
        if Link:
            ...
            if self.result == {}:
                ...
                with connect_mysql() as coon:
                    print('\n网址超链接入库中:'+self.url+':'+str(self.result))
...
def Start(start):
    ...
    if start == 1:
        ...
        print('[Start Scan] 开始SQL注入扫描\n')
        os.system(sql_start)
        time.sleep(1)

        print('[Start Scan] 开始XSS漏洞扫描\n')
        os.system(xss_start)
        time.sleep(1)

        print('[Start Scan] 开始URL跳转扫描\n')
        os.system(url_start)
        time.sleep(1)
    #
    #
...
if __name__ == '__main__':
    ...
    if start_on == 0:
        ...
        if len(All_Urls) > 1000:
            print('----------------------------------------------------')
            print('----目标数量超过1000，将从中提取1000个网址加载扫描----')
            print('----------------------------------------------------')
