...
def Common_Database(document,common,content,url,sqlurl):
    ...
    try:
        ...
        try:
            ...
            writedata(str(e))
        document.add_paragraph('网站漏洞复现【SQLMAP版本号1.2.9.39#dev】', 'Subtitle')
        document.add_paragraph('高版本的boundaries.xml没有了针对模糊查询(%)的测试，并且必须手动设置json的某个参数为*才能对这个参数进行注入，否则payload直接跟在json后导致无法注入','DaHei')
