...
def scan(url):
    ...
    if content:
        if 'zookeeper.version' in content:
    ...
    content = Get_Resp(urls)
    if content:
        if '/resin-doc/examples/index.xtp' in content:
    ...
    content = Get_Resp(urls)
    if content:
        if 'servlet/RequestParamExample' in content:
    ...
    content = Get_Resp(urls)
    if content:
        if 'flavor=URL,type=DeploymentScanner' in content:
