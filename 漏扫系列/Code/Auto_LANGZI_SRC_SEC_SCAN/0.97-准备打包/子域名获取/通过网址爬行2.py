...
black_list = list(set([x.strip() for x in open('black_list', 'r', encoding='utf-8').readlines()]))
        ...
        try:
            ...
            if b'Service Unavailable' not in r.content and b'The requested URL was not found on' not in r.content and b'The server encountered an internal error or miscon' not in r.content:
                if r.status_code == 200 or r.status_code == 301 or r.status_code==302:
                    ...
                    encodings = requests.utils.get_encodings_from_content(r.text)[0]
                    contents = r.content.decode(encodings,'replace')
                    title = Get_Tltile(contents)
                    ...
                    try:
                        ...
                        server = '获取失败'
                    #if '.gov.cn' not in real_url and '.edu.cn' not in real_url:
                    try:
                        with connect_mysql() as cursor:
                            print(url,title)
                            sqls = 'insert into Sec_Urls(url,title,power,server,content) values ("{}","{}","{}","{}","{}")'.format(
                                pymysql.escape_string(urlparse(real_url).scheme + '://' + urlparse(real_url).netloc), pymysql.escape_string(title).replace(' ','').replace('\n','').replace('\\',''), power, server,pymysql.escape_string(str(contents.strip().replace(' ','').replace('\n','').replace('\\',''))))
                            # print(sqls)
                            cursor.execute(sqls)
                    except Exception as e:
                        print(e)
...
def Get_Linkss(sem):
    ...
    while 1:
        try:
            ...
            if link:
                for url in link:
                    #if '.gov.cn' not in url and '.edu.cn' not in url:
                        # with connect_mysql() as coon:
...
def test_start():
    with connect_mysql() as conn:
        sql1 = 'select count(id) from Sec_Index where subdomain=0'
        conn.execute(sql1)
        res = conn.fetchone()
        if res == None:
