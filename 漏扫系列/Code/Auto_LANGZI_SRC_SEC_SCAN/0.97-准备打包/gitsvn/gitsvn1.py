...
def main_svn(url):
    ...
    document.add_picture('image/codee.png', width=Inches(7))
    ...
    if '.gov.cn' not in url and '.edu.cn' not in url:
        ...
        async with asyncio.Semaphore(1000):
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
                try:
                    async with session.get(url+'/.svn/entries',headers=headers,timeout=15,ssl=False) as resp:
                        res = await resp.text()
                        if 'dir' in res and 'svn://' in res:
                            async with aiofiles.open('git_svn_result.txt', 'a+',encoding='utf-8')as f:
                                await f.write(url.rstrip('/')+'/.svn/entries' + '\n')
                except Exception as e:
                    #print(e)
                    pass

                try:
                    async with session.get(url+'/.git/config',headers=headers,timeout=15,ssl=False) as resp:
                        res1 = await resp.text()
                        if 'repositoryformatversion' in res1:
                            async with aiofiles.open('git_svn_result.txt', 'a+',encoding='utf-8')as f:
                                await f.write(url.rstrip('/')+'/.git/config' + '\n')
                except Exception as e:
                    #print(e)
                    pass
...
async def main(urls):
    async with aiomultiprocess.Pool() as pool:
        await pool.map(runs,urls)


def test_start():
    with connect_mysql() as conn:
        sql1 = 'select count(id) from Sec_Index where gitsvn=0'
        conn.execute(sql1)
        res = conn.fetchone()
        if res == None:
...
if __name__ == '__main__':
    ...
    if os.path.exists('git_svn_result.txt'):
        os.remove('git_svn_result.txt')
    ...
    while 1:
        with connect_mysql() as conn:
            ...
            if res == 0:
                print('数据库全部URL对GITSVN泄露扫描完毕')
                #os.system('pause')
                sys.exit(0)
            if res > 800:
                sql2 = 'select url from Sec_Index where gitsvn=0 limit 800'
                sql3 = 'update Sec_Index set gitsvn=1 where gitsvn=0 limit 800'
                conn.execute(sql2)
                ...
                sql2 = 'select url from Sec_Index where gitsvn=0 limit '+str(res)
                sql3 = 'update Sec_Index set gitsvn=1 where gitsvn=0 limit '+str(res)
                result = [x[0] for x in (conn.fetchall())]
                print('任务计划:'+str(result))
                conn.execute(sql3)
                loop = asyncio.get_event_loop()
