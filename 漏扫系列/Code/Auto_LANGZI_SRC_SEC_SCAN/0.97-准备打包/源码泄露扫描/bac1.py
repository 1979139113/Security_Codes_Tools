...
async def main(urls,backs):
    ...
    for b in backs:
        for u in urls:
            if '.gov.cn' not in u and '.edu.cn' not in u:
    ...
    while 1:
        with connect_mysql() as conn:
            ...
            if res == 0:
                ...
                os.system('pause')
            if res > 800:
                sql2 = 'select url from Sec_Index where backup=0 limit 800'
                sql3 = 'update Sec_Index set backup=1 where backup=0 limit 800'
                conn.execute(sql2)
