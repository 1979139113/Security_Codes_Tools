...
list_001 = []
fck=['/fckeditor/editor/filemanager/browser/default/connectors/test.html','/fckeditor/editor/filemanager/connectors/test.html','/FCKeditor/editor/filemanager/connectors/uploadtest.html','/FCKeditor/editor/filemanager/upload/test.html','/fckeditor/editor/filemanager/browser/default/browser.html','/FCKeditor/editor/fckeditor.html']
        ...
        if r_svn.status_code == 200:
            ...
            try:
                ...
                if 'dir' in r_svn_1.content and 'svn://' in r_svn_1.content:
                    ...
                    if r_svn_1.encoding == 'ISO-8859-1':
                        encodings = requests.utils.get_encodings_from_content(r_svn_1.text)
                        if encodings:
                            encoding = encodings[0]
                        else:
                            encoding = r_svn_1.apparent_encoding
                    encode_content = r_svn_1.content.decode(encoding, 'replace').encode('utf-8', 'replace')
                    ...
                    try:
                        ...
                        cur_svn.execute(sql_svn, (str(r_svn_1.url), str('svn'), str(svn_title),str(timenow)))
                        coon_svn.commit()
                        cur_svn.close()
                        coon_svn.close()
        ...
        if r_git.status_code == 200:
            try:
                ...
                if 'repositoryformatversion' in r_git_1.content:
                    ...
                    if r_git_1.encoding == 'ISO-8859-1':
                        encodings = requests.utils.get_encodings_from_content(r_git_1.text)
                        if encodings:
                            encoding = encodings[0]
                        else:
                            encoding = r_git_1.apparent_encoding
                    encode_content = r_git_1.content.decode(encoding, 'replace').encode('utf-8', 'replace')
                    ...
                    try:
                        ...
                        cur_git.execute(sql_git, (str(r_git_1.url), str('git'), str(git_title),str(timenow)))
                        coon_git.commit()
                        cur_git.close()
                        coon_git.close()
        ...
        if r_info.status_code == 200:
            try:
                ...
                if '<web-app' in r_info_1.content:
                    if r_info_1.encoding == 'ISO-8859-1':
                        encodings = requests.utils.get_encodings_from_content(r_info_1.text)
                        if encodings:
                            encoding = encodings[0]
                        else:
                            encoding = r_info_1.apparent_encoding
                    encode_content = r_info_1.content.decode(encoding, 'replace').encode('utf-8', 'replace')
                    ...
                    try:
                        ...
                        cur_info.execute(sql_info, (str(r_info_1.url), str('WEB-INF/web.xml'), str(web_title),str(timenow)))
                        coon_info.commit()
                        cur_info.close()
                        coon_info.close()
            ...
            if r_domain.status_code == 200:
                try:
                    if int(r_domain.headers["Content-Length"]) > 2000000:
                        if r_domain.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain.apparent_encoding
                        encode_content = r_domain.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_0.status_code == 200:
                try:
                    if int(r_domain_0.headers["Content-Length"]) > 2000000:
                        if r_domain_0.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_0.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_0.apparent_encoding
                        encode_content = r_domain_0.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_1.status_code == 200:
                try:
                    if int(r_domain_1.headers["Content-Length"]) > 2000000:
                        if r_domain_1.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_1.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_1.apparent_encoding
                        encode_content = r_domain_1.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_3.status_code == 200:
                try:
                    if int(r_domain_3.headers["Content-Length"]) > 2000000:
                        if r_domain_3.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_3.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_3.apparent_encoding
                        encode_content = r_domain_3.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_2.status_code == 200:
                try:
                    if int(r_domain_2.headers["Content-Length"]) > 2000000:
                        if r_domain_2.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_2.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_2.apparent_encoding
                        encode_content = r_domain_2.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_4.status_code == 200:
                try:
                    if int(r_domain_4.headers["Content-Length"]) > 2000000:
                        if r_domain_4.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_4.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_4.apparent_encoding
                        encode_content = r_domain_4.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_5.status_code == 200:
                try:
                    if int(r_domain_5.headers["Content-Length"]) > 2000000:
                        if r_domain_5.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_5.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_5.apparent_encoding
                        encode_content = r_domain_5.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_domain_6.status_code == 200:
                try:
                    if int(r_domain_6.headers["Content-Length"]) > 2000000:
                        if r_domain_6.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_domain_6.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_domain_6.apparent_encoding
                        encode_content = r_domain_6.content.decode(encoding, 'replace').encode('utf-8', 'replace')
            ...
            if r_rar.status_code == 200:
                try:
                    if int(r_rar.headers["Content-Length"]) > 2000000:
                        if r_rar.encoding == 'ISO-8859-1':
                            encodings = requests.utils.get_encodings_from_content(r_rar.text)
                            if encodings:
                                encoding = encodings[0]
                            else:
                                encoding = r_rar.apparent_encoding
                        encode_content = r_rar.content.decode(encoding, 'replace').encode('utf-8', 'replace')
        ...
        try:
            ...
            if req_fck_0.status_code == 200:
                ...
                if ('title>FCKeditor - ') in req_fck_1.content and req_fck_1.status_code==200:
                    if req_fck_1.encoding == 'ISO-8859-1':
                        encodings = requests.utils.get_encodings_from_content(req_fck_1.text)
                        if encodings:
                            encoding = encodings[0]
                        else:
                            encoding = req_fck_1.apparent_encoding
                    encode_content = req_fck_1.content.decode(encoding, 'replace').encode('utf-8', 'replace')
        ...
        for inj in payloads:
            ...
            for key, vlue in sql_errors.iteritems():
                if str(key) in r_inj.content:
                    if r_inj.encoding == 'ISO-8859-1':
                        encodings = requests.utils.get_encodings_from_content(r_inj.text)
                        if encodings:
                            encoding = encodings[0]
                        else:
                            encoding = r_inj.apparent_encoding
                    encode_content = r_inj.content.decode(encoding, 'replace').encode('utf-8', 'replace')
                    ...
                    try:
                        ...
                        sql_sql_2 = "INSERT INTO url_sqlinj (url,urltitle,sqldatabase,datatime) VALUES (%s,%s,%s)"
                        cur_sql_2.execute(sql_sql_2, (str(url_inj), str(sql_title),str(vlue), str(timenow)))
                        coon_sql_2.commit()
                        cur_sql_2.close()
                        coon_sql_2.close()
                        time.sleep(random.randint(2, 6))
        ...
        for xx_0 in list_001:
            ...
            try:
                ...
                if urlxieru.status_code == 200:
                    ...
                    cur.execute(sql)
                    coon.commit()
                    sql001 = "INSERT INTO url_check(url,cmsscan,rarscan,sqlscan,st2scan,editorscan,portscan,datatime) select '%s','%s','%s','%s','%s','%s','%s','%s' from dual where '%s' not in (select url from url_check)"%((str(xx),0,0,0,0,0,0,str(timenow),str(xx)))
                    cur.execute(sql001)
                    coon.commit()
                else:
                    pass
                    ...
                    try:
                        if 'www' in xxx.split('http://')[1]:
                            ...
                            if xieru2.status_code == 200:
                                timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                sqlcaiji1 = "INSERT INTO url_check(url,cmsscan,rarscan,sqlscan,st2scan,editorscan,portscan,datatime) select '%s','%s','%s','%s','%s','%s','%s','%s' from dual where '%s' not in (select url from url_check)" % ((str(xxx), 0, 0,0, 0,0,0,str(timenow), str(xxx)))
                            ...
                            if xieru2_0.status_code == 200:
                                timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                sqlcaiji1_0 = "INSERT INTO url_2check(url,cmsscan,rarscan,sqlscan,st2scan,editorscan,portscan,datatime) select '%s','%s','%s','%s','%s','%s','%s','%s' from dual where '%s' not in (select url from url_check)" % ((str(xxx), 0, 0, 0, 0, 0, 0, str(timenow), str(xxx)))
