...
def scan(url):
    for x in st:
        ...
        try:
            ...
            r = requests.head(url=urlx,headers=headers,allow_redirects=False,timeout=3)
            #print r.url + ': ' + str(r.status_code)
            if r.status_code==200:
                r2 = requests.get(url=urlx,headers=headers,allow_redirects=False,timeout=5)
                if '.action' in r2.content:
            ...
            print e
uu = list(set([i.replace("\n","") for i in open("ok.txt","r").readlines()]))
for x in uu:scan(x)