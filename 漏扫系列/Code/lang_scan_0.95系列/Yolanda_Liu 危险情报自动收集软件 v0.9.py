...
def scan_cms(url):
    try:
        ...
        print str(threading.current_thread().name) + ' ' + str(req3.url) + '  ' + str(req3.status_code)
    ...
    try:
        ...
        for xx_0 in list_001:
            ...
            try:
                ...
                print str(threading.current_thread().name) + ' ' + str(urlxieru.url) + '  ' + str(urlxieru.status_code)
                ...
                try:
                    ...
                    for xx_x0 in list_002_1:
                        ...
                        try:
                            ...
                            print str(threading.current_thread().name) + ' ' + str(xieru2.url) + '  ' + str(xieru2.status_code)
...
def sqltiqu():
    ...
    try:
        ...
        coonsql2.commit()
        try:
            curscms = cursql2.fetchone()
            for xx in curscms:
                xxx1cms = xx.replace("('","").replace("',)","")
                list_zairusql.append(xxx1cms)
            cursql2.execute(sql1)
            coonsql2.commit()
            cursql2.close()
            coonsql2.close()
            for xxxxxxx in list_zairusql:
                scan_sql(xxxxxxx)
        except Exception,e:
            pass
    except Exception,e:
