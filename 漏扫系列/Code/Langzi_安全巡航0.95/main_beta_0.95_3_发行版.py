...
def GET(url, level):
    writedata('[SCAN XSS GET]  ' + str(url))
    result = {}
...
def POST(domains, data, level):
    writedata('[SCAN XSS POST]  ' + str(domains + '&' + data))
    result = {}
...
system_passwords_list = ['admin12345', 'admin1234', 'ubuntu', 'root', 'a123456', '123465', '123456789', '111111',
                         '888888', '88888888', '666666', '123456']
...
mysql_password_list = ['root', '123456', 'admin1234', 'a123456', '12345', '123456789', '111111', '888888', '88888888',
                       '666666']
...
mssql_password_list = ['root', 'Sa123', 'Sa123456', 'Sa1234', '123456', 'admin1234', 'a123456', '12345', '123456789',
                       '111111', '888888', '88888888', '666666']
...
postql_password_list = ['root', 'postgres', 'admin', '123456', 'admin1234', 'a123456', '12345', '123456789', '111111',
                        '888888', '88888888', '666666']
...
def scan_database(url):
    ...
    for username in mysql_user_list:
        for password in mysql_password_list:
            writedata('[SCAN MySQL]  ' + str(ip + ':' + str(3306) + '|' + username + ':' + password))
            try:
    ...
    for username in mssql_user_list:
        for password in mssql_password_list:
            writedata('[SCAN MSSQL]  ' + str(ip + ':' + str(1433) + '|' + username + ':' + password))
            try:
    ...
    for username in postql_user_list:
        for password in postql_password_list:
            writedata('[SCAN PostSQL]  ' + str(ip + ':' + str(5432) + '|' + username + ':' + password))
            try:
    ...
    for username in system_user_list:
        for password in system_passwords_list:
            writedata('[SCAN SSH]  ' + str(ip + ':' + str(22) + '|' + username + ':' + password))
            try:
            ...
            try:
                writedata('[SCAN FTP]  ' + str(ip + ':' + str(21) + '|' + username + ':' + password))
                ftp = FTP(ip)
                ftp.connect(ip, 21)
            ...
            try:
                writedata('[SCAN TelNET]  ' + str(ip + ':' + str(23) + '|' + username + ':' + password))
                res = ''
                s = socket.socket()
                s.connect((str(ip), 23))
                s.send('langzi \n')
    ...
    for username in system_user_list:
        for password in system_passwords_list:
            writedata('[SCAN SMB]  ' + str(ip + ':' + str(445) + '|' + username + ':' + password))
            try:
...
def scan_url(url):
    ...
    try:
        UA = random.choice(headerss)
        headers = {'User-Agent': UA}
        writedata('[SCAN CouchDB]  ' + str(url + '/_config'))
    ...
    try:
        UA = random.choice(headerss)
        headers = {'User-Agent': UA}
        writedata('[SCAN Tomcat example]  ' + url + '/servlets-examples/')
    ...
    try:
        ...
        r_.append(r5)
        for r_r in r_:
            writedata('[SCAN Resin viewfile]  ' + r_r)
            try:
                rxr = requests.get(url=r_r, headers=headers, timeout=8)
            ...
            try:
                for uuser in user_list:
                    for ppass in password_list:
                        writedata('[SCAN Weblogic]  ' + r_r + ':' + uuser + '|' + ppass)
        ...
        for r_r in r_:
            try:
                ...
                if 'onkeypress="if(event.keyCode==13)' in rxr.content:
                    for uuser in user_list:
                        for ppass in password_list:
                            writedata('[SCAN Tomcat]  ' + r_r + ':' + uuser + '|' + ppass)
    ...
    try:
        ...
        for uuser in user_list:
            for ppass in password_list:
                writedata('[SCAN Wordpress]  ' + url + ':' + uuser + '|' + ppass)
        ...
        for r_r in r_:
            try:
                ...
                if 'Documentation.html' in rxr.content:
                    for uuser in user_list:
                        for ppass in password_list:
                            writedata('[SCAN PHPmyadmin]  ' + r_r + ':' + uuser + '|' + ppass)
...
def scan_ip(url):
    ...
    if ip == None:
        return None
    result = []
    writedata('[SCAN IP VLUN]  ' + 'Mongodb数据库未授权访问漏洞 : ' + str(ip) + ':27017')
    writedata('[SCAN IP VLUN]  ' + 'Mongodb数据库未授权访问漏洞 : ' + str(ip) + ':27018')
    try:
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Redis数据库未授权访问漏洞 : ' + str(ip) + ':6379')
        s = socket.socket()
    ...
    try:
        ...
        if "Authentication" in result:
            for pass_ in password_list:
                ...
                s.send("AUTH %s\r\n" % (pass_))
                result = s.recv(1024)
                writedata('[SCAN IP VLUN]  ' + 'Redis弱口令漏洞 : ' + str(ip) + ':6379|' + str(pass_))
                if '+OK' in result:
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'ZooKeeper未授权访问漏洞 : ' + str(ip) + ':2181')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Elasticsearch未授权访问漏洞 : ' + str(ip) + ':9200')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Memcache未授权访问漏洞 : ' + str(ip) + ':11211')
    ...
    try:
        ...
        r_.append(r4)
        for r_r in r_:
            writedata('[SCAN IP VLUN]  ' + 'IIS短文件名漏洞 : ' + str(r_r))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'IIS WebDav任意文件上传漏洞 : ' + str(url))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Docker未授权访问漏洞 : ' + str(ip) + ':2375/containers/json')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'CouchDB未授权访问漏洞 : ' + str('http://' + str(ip) + '/_config'))
    ...
    try:
        ...
        r_.append(r4)
        for r_r in r_:
            writedata('[SCAN IP VLUN]  ' + 'Jenkins未授权访问漏洞 : ' + str(r_r))
    ...
    try:
        ...
        writedata('[SCAN IP VLUN]  ' + 'ActiveMQ任意文件上传漏洞 : ' + str(url))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'WebServer任意文件读取漏洞 : ' + str(ip) + ':80')
        ...
        s.close()
    try:
        writedata('[SCAN IP VLUN]  ' + 'WebServer任意文件读取漏洞 : ' + str(ip) + ':443')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'WebServer任意文件读取漏洞 : ' + str(ip) + ':8080')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Fast-Cgi文件读取漏洞 : ' + str(ip) + ':9000')
        ...
        for r_r in r_:
            try:
                ...
                if 'Documentation.html' in rxr.content:
                    for uuser in user_list:
                        for ppass in password_list:
                            writedata('[SCAN IP VLUN]  ' + 'PHPmyadmin弱口令漏洞 : ' + str(
                                r_r + ':' + str(str(uuser) + '|' + str(ppass))))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'HTTP.sys远程代码执行漏洞 : ' + str(ip) + ':80')
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'HTTP.sys远程代码执行漏洞 : ' + str(ip) + ':443')
        ...
        s.close()
        if 'Requested Range Not Satisfiable' in data and 'Server: Microsoft' in data:
            result.append('HTTP.sys远程代码执行漏洞 : ' + str(ip) + ':443')
    except Exception, e:
        pass
    finally:
        s.close()
    ...
    try:
        url = 'http://' + ip + ":" + str(80)
        writedata('[SCAN IP VLUN]  ' + '.NET Padding Oracle信息泄露 : ' + str(url))
    ...
    try:
        ...
        r_.append(r6)
        for r_r in r_:
            writedata('[SCAN IP VLUN]  ' + 'Resin viewfile远程文件读取漏洞 : ' + str(r_r))
    ...
    try:
        ...
        writedata('[SCAN IP VLUN]  ' + 'Tomcat example 应用信息泄漏漏洞:' + rrrx.url.strip('/'))
    ...
    try:
        ...
        writedata('[SCAN IP VLUN]  ' + 'Jboss 认证绕过漏洞 : ' + str(webshell_url))
    ...
    try:
        ...
        r_.append(r4)
        for r_r in r_:
            writedata('[SCAN IP VLUN]  ' + 'JBoss后台上传漏洞 : ' + str(r_r))
    ...
    try:
        ...
        for r_r in r_:
            try:
                for uuser in user_list:
                    for ppass in password_list:
                        writedata('[SCAN IP VLUN]  ' + 'Weblogic弱口令漏洞 : ' + str(
                            r_r + ':' + str(str(uuser) + '|' + str(ppass))))
        ...
        for r_r in r_:
            try:
                ...
                if 'JOnAS Administration' in rxr.content:
                    for uuser in user_list:
                        for ppass in password_list:
                            writedata('[SCAN IP VLUN]  ' + 'JOnAS弱口令漏洞 : ' + str(
                                r_r + ':' + str(str(uuser) + '|' + str(ppass))))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Weblogic CVE-2018-2628 : ' + str(ip) + ':7001')
    ...
    try:
        ...
        for xxixx in r_:
            ...
            for uuser in user_list:
                for ppass in password_list:
                    writedata('[SCAN IP VLUN]  ' + 'Glassfish弱口令漏洞 : ' + str(
                        xxixx + ':' + str(str(uuser) + '|' + str(ppass))))
    ...
    try:
        ...
        for uuser in user_list:
            for ppass in password_list:
                writedata(
                    '[SCAN IP VLUN]  ' + 'Wordpress弱口令漏洞 : ' + str(url + ':' + uuser + '|' + ppass))
    ...
    try:
        writedata('[SCAN IP VLUN]  ' + 'Axis2任意文件读取漏洞 : ' + str(ip))
        url = "http://%s:%d" % (ip, int(8080))
    ...
    try:
        ...
        r_.append(r11111)
        for r_r in r_:
            writedata('[SCAN IP VLUN]  ' + '服务器探针信息泄露:' + r_r)
...
def write_success(x):
    with open('result.txt', 'a+')as a:
        a.write(str(time.strftime('%Y-%m-%d:%H:%M:%S   ', time.localtime())) + x + '\n')
...
def live_check(url):
    try:
        ...
        writedata('[WARNING ERROR]  ' + str(e))
        writedata('[WARNING ERROR]  Connection  ' + url + '  Failed')
        return False
...
def run(url):
    if 'gov.cn' in url or 'edu.cn' in url:
        ...
        # 网站的服务器类型
        # url.replace('https://', '').replace('http://', '').replace('/', '').replace(
        #         'www.', '')

        # 文件名类型设置

        try:
            filename = 'result/' + title.decode('utf-8') + '__VlunReport.txt'
            with open(filename, 'a+')as e:
                e.write('                             【 网站信息 】 \n')
        except:
            try:
                filename = 'result/' + title + '__VlunReport.txt'
                with open(filename, 'a+')as e:
                    e.write('                             【 网站信息 】 \n')
            except:
                filename = 'result/' + url.replace('https://', '').replace('http://', '').replace('/', '').replace('www.', '') + '__VlunReport.txt'
                with open(filename, 'a+')as e:
                    e.write('                             【 网站信息 】 \n')
        ...
        write_result(t='所属  IP', c=ip, filename=filename)
...
if __name__ == '__main__':
    ...
    print unicode('     LangZi安全巡航_Version_0.95_beta_多进程版(4进程)', 'utf-8')
    print unicode('     LangZi安全巡航_Version_0.95_beta_跨年礼物专版', 'utf-8')
