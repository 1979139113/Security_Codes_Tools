...
def first_scan(url):
    ...
    ip = socket.gethostbyname(str(url.split('//')[1].split('/')[0]))
    print ip
    first_ip1 = ip.replace(ip.split('.')[3],'')
    for ii in range(1,254):
        first_ipp = first_ip1 +str(ii)
        #print '[+] GET : '+ str(first_ipp)
        try:
            UA = random.choice(headerss)
            headers = {'User-Agent': UA}
            url_secend = 'https://dns.aizhan.com/'+''.join(first_ipp) + '/'
            try:
                print '[+] Scan URL >>>>> https://dns.aizhan.com/' + str(first_ipp) + '/'
                time.sleep(3)
                p = requests.get(url=str(url_secend), headers=headers, timeout=20).content
                if isinstance(p, unicode):
                    ...
                    try:
                        r = re.findall('<a href="(.*?)/" rel="nofollow" target="_blank">', a)
                        if r:
                            for x in r:
                                print x
                                # print y.replace('<span>', '').replace('\n', '').replace('</span>', '').replace(
                                #     '<td', '').replace('</td>', '').replace('	', '').replace(
                                #     '												', '').replace(
                                #     '					', '')
                                ...
                                try:
                                    ...
                                    cur_svn.execute(sql_svn,(str(x), str(first_ipp), '0',str(timenow)))
                                    coon_svn.commit()
                                    cur_svn.close()
                                    coon_svn.close()
                                except Exception, e:
...
def cmstiqu():
    try:
        ...
        print e
        print unicode('C段与旁站数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('C段与旁站数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('C段与旁站数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('C段与旁站数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('C段与旁站数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
...
def cmstiqu2():
    try:
        ...
        print e
        print unicode('外链数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('外链数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('外链数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('外链数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
        print unicode('外链数据库无数据....等待2分钟后自动继续提取数据.....', 'utf-8')
