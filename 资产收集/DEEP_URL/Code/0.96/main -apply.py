...
if titles.find('&') > 0:
    ...
    for x, y in enumerate(d):
        list_for_and_title.append(y.decode('gbk'))
        print '     ' + str(x) + ':' + unicode(y, 'gbk')
if titles.find('|') > 0:
    d = titles.split('|')
    print unicode('标题白名单 : 标题仅需存在如下规则之一即可被添加', 'gbk')
    for x, y in enumerate(d):
        list_for_or_title.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
if titles == 'None':
...
if titles.find('&') <0 and titles.find('|') <0 and titles !='None':
    ...
    list_for_and_title.append(titles.decode('gbk'))
if black_titles.find('&') > 0:
    d = black_titles.split('&')
    print unicode('标题黑名单 : 标题中需同时存在如下规则才会被排除', 'gbk')
    for x, y in enumerate(d):
        list_for_and_black_title.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
if black_titles.find('|') > 0:
    d = black_titles.split('|')
    print unicode('标题黑名单 : 标题仅需存在如下规则之一即可被排除', 'gbk')
    for x, y in enumerate(d):
        list_for_or_black_title.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
...
if black_titles.find('&')<0 and black_titles.find('|') <0 and black_titles !='None':
    ...
    list_for_or_black_title.append(black_titles.decode('gbk'))
...
if contents.find('&') > 0:
    ...
    for x, y in enumerate(d):
        list_for_and_content.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
if contents.find('|') > 0:
    d = contents.split('|')
    print unicode('网页白名单 : 网页仅需存在如下规则之一即可被添加', 'gbk')
    for x, y in enumerate(d):
        list_for_or_content.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
if contents == 'None':
...
if contents.find('&') <0 and contents.find('|') <0 and contents !='None':
    ...
    list_for_and_content.append(contents.decode('gbk'))

if black_contents.find('&') > 0:
    d = black_contents.split('&')
    print unicode('网页黑名单 : 网页中需同时存在如下规则才会被排除', 'gbk')
    for x, y in enumerate(d):
        list_for_and_black_content.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
if black_contents.find('|') > 0:
    d = black_contents.split('|')
    print unicode('网页黑名单 : 网页仅需存在如下规则之一即可被排除', 'gbk')
    for x, y in enumerate(d):
        list_for_or_black_content.append(y.decode('gbk'))
        print  '     ' + str(x) + ':' + unicode(y, 'gbk')
...
if black_contents.find('&')<0 and black_contents.find('|') <0 and black_contents !='None':
    ...
    list_for_or_black_content.append(black_contents.decode('gbk'))
