...
if white == 0:
    print unicode('白名单与项 : 关闭', 'gbk')
    log((' [*] 程序运行 : 白名单与项 : 关闭').decode('gbk'))
if white == 1:
...
if forever == 0:
    print unicode('采集次数项 : 关闭', 'gbk')
    log((' [*] 程序运行 : 采集次数项 : 关闭').decode('gbk'))
if forever == 1:
    print unicode('采集次数项 : 采集 1 次', 'gbk')
    log((' [*] 程序运行 : 采集次数项 : 开启').decode('gbk'))
if forever >1:
    print unicode('采集次数项 : 采集 %s 次', 'gbk')%str(forever)
    log((unicode(' [*] 采集次数项 : 采集 %s 次', 'gbk')%str(forever)))
    ...
    if white == 0:
        ...
        if list_for_and_black_url !=[]:
            ...
            if i == len(list_for_and_black_url):
                print url + ' : ' + unicode('排除原因 : 网址中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_url).decode('gbk')
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 网址中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_url).decode('gbk')))
                return False
        if list_for_and_url != []:
            i = 0
        ...
        if list_for_and_black_title != []:
            ...
            if i == len(list_for_and_black_title):
                print url + ' : ' + unicode('排除原因 : 标题中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_title).decode('gbk')
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 标题中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_title).decode('gbk')))
                return False
        ...
        if list_for_and_black_content != []:
            ...
            if i == len(list_for_and_black_content):
                print url + ' : ' + unicode('排除原因 : 网页中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_content).decode('gbk')
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 网页中同时存在 : %s', 'gbk') % '|'.join(list_for_and_black_content).decode('gbk')))
                return False
        ##--------------------------开啥车
        ...
        if list_for_or_url !=[]:
            ...
            if i == 0:
                ...
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 网址中皆不存在 : %s', 'gbk') % '|'.join(list_for_or_url).decode('gbk')))
                return False
        if list_for_or_title !=[]:
            ...
            if i == 0:
                ...
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 标题中皆不存在 : %s', 'gbk') % '|'.join(list_for_or_title).decode('gbk')))
                return False
        if list_for_or_content !=[]:
            ...
            if i == 0:
                ...
                log((' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 网页中皆不存在 : %s', 'gbk') % '|'.join(list_for_or_content).decode('gbk')))
                return False
            ...
            try:
                ...
                if r.status_code == 200:
                    try:
                        with open(result_name, 'a+')as a:
                            a.write(xx + '\n')
                            print xx + ' : ' + unicode('符合要求，网址写入本地', 'gbk')
                            log((' [+] 网址写入 : ' + xx).decode('gbk'))
                    except Exception, e:
                        log((' [*] 异常状态 : ' + str(e)).decode('gbk'))
                        pass
                else:
