...
def get_url(url):
    ...
    if list_for_or_url != []:
        ...
        if i == 0:
            print url + ' : ' + unicode('排除原因 : 网址中皆不存在 : %s', 'gbk') % '|'.join(list_for_or_url).decode('gbk')
            log((
                ' [-] 网址排除 : ' + url + ' : ' + unicode('排除原因 : 网址中皆不存在 : %s', 'gbk') % '|'.join(list_for_or_url).decode(
                    'gbk')))
            return False
        else:
            try:
    ...
    try:
        ...
        if setstart == 1:
            keywords = urllib.quote(
                str(raw_input(unicode('输入关键词,采集网址。(例子:inurl:php/浪子帅逼/AT2@32D^%$):', 'gbk').encode('gbk'))))
            ...
            if track == 0:
                ...
                if titles != 'None' or black_titles != 'None' or contents != 'None' or black_contents != 'None':
                    try:
                        result_name = result_dir + '/' +str(time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())).replace(' ',
                                                                                                        '') + '.txt'
        ...
        log((' [*] 异常状态 : ' + str(e)).decode('gbk'))
    nn = result_dir + '/result.txt'
    f = list(set([x.replace('\n', '').replace(' ', '') for x in open(nn, 'r').readlines()]))
    with open(nn,'w')as a:
        pass
    with open(nn,'a+')as a:
        for x in f:
