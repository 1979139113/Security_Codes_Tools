...
create_sql=[
    ...
    'use ' + Dbname,
    'create table if not EXISTS url_index(id int primary key auto_increment,url varchar(80),urlget varchar(1),datatime varchar(80))charset=utf8;',
    'create table if not EXISTS url_cms(id int primary key auto_increment,url varchar(80),urlway varchar(100),cmstype varchar(100),urltitle varchar(100),datatime varchar(80))charset=utf8;',
    'alter table url_index index index_name (url_get)'
]
port = int(port)
time.sleep(5)
try:  # line:175
...
print (unicode(' *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*', 'utf-8'))
time.sleep(0.3)
print (unicode(' *-*-   [+] 首次使用输入 1    (自动安装数据库,输入关键词采集网址)', 'utf-8'))  # line:185
time.sleep(0.5)
print ('\n')  # line:186
print (unicode(' *-*-   [+] 非首次使用输入 2   (从数据库继续提取网址扫描)', 'utf-8'))  # line:187
time.sleep(0.7)
print ('\n')  # line:188
print (unicode(' *-*-   [+] 删除数据库输入 0   (也可用于删除数据库重新扫描)', 'utf-8'))  # line:189
time.sleep(1)
print (unicode(' *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*', 'utf-8'))
print ('\n')

New_start = raw_input(unicode('请选择启动方式(选项:0/1/2):', 'utf-8').encode('gbk'))  # line:190
if New_start == '1':  # line:192
    print (unicode(' [+] 开始创建数据库.......', 'utf-8'))  # line:193
    time.sleep(3)  # line:194
    try:  # line:195
        coon = pymysql.connect(user=user, passwd=passwd, host=host, port=port,charset='utf8')  # line:196
        cur = coon.cursor()  # line:197
        for i in create_sql:  # line:198
            print (i)  # line:199
            time.sleep(1)  # line:200
            cur.execute(i)  # line:201
            coon.commit()  # line:202
            print ('Execute SQL Success.....')
        cur.close()  # line:203
        coon.close()  # line:204
        print ('\n')
        print (unicode(' [+] 数据库创建成功.....', 'utf-8'))  # line:205
    except:  # line:206
        pass  # line:207
if New_start == '2':  # line:208
    print ('\n')
    print (unicode(' [+] 开始从数据库提取网址扫描......', 'utf-8'))  # line:209
if New_start == '0':  # line:210
    time.sleep(1)  # line:211
    coon = pymysql.connect(user=user, passwd=passwd, host=host, port=port,charset='utf8')  # line:212
    cur = coon.cursor()  # line:213
    cur.execute('drop database ' + Dbname)  # line:214
    coon.commit()  # line:215
    cur.close()  # line:216
    coon.close()  # line:217
    print (unicode(' [+] 数据库删除成功......', 'utf-8'))  # line:218
    time.sleep(1)  # line:219
    print (unicode(' [+] 我先撤了，要继续扫描就重开......', 'utf-8'))  # line:220
    time.sleep(3)  # line:221
    os._exit(0)  # line:222
headerss = [
                ...
                if xx.find('link') > 0:
                    try:
                        ...
                        if rxr.status_code == 200:
                            print (' [*] First Found Url: ' + rxr.url.split('://')[0] + '://' + rxr.url.split('://')[1].split('/')[0])
...
if New_start =='1':
    print ('\n')
    keywords = urllib.quote(str(raw_input(unicode('输入关键词,采集网址。(例子:inurl:php/浪子帅逼/AT2@32D^%$):','utf-8').encode('gbk'))))
    scan_baidu(str(keywords))
def get_result():
    while 1:
        time.sleep(600)
        try:
    ...
    try:
        ...
        try:
            ...
            for xx_x0 in list_002_1:
                xxx = xx_x0.strip('/')
                #if xxx.find('.gov.cn')>0 or xxx.find('.edu.cn')>0:
                if xxx.find('.')>2 or xxx.find('//www.')<0 or xxx.find('=') >0:
                    pass
                else:
                    UA = random.choice(headerss)
                    headers = {'User-Agent': UA}
                    xieru2 = requests.head(url=str(xxx), headers=headers, timeout=5)
                    print (str('URL Scan Thread - Main') + ' ' + str(xieru2.url).strip('/') + '  ' + str(xieru2.status_code))
                    if xieru2.status_code == 200:
                        try:
                            cooncaiji1 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname,port=port,charset='utf8')
    ...
    try:
        ...
        for key,valu in body.iteritems():
            if key in req3.content:
                try:
                    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    coon_cms_1 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname,port=port,charset='utf8')
    ...
    try:
        ...
        for x_x in robots:
            if x_x in req2.content:
                try:
                    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    coon_cms_2 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname,port=port,charset='utf8')
                    ...
                    try:
                        ...
                        if  x['md5'] == '':
                            ...
                            if d>0:
                                try:
                                    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                    coon_cms_3 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname,port=port, charset='utf8')
    ...
    while 1:
        try:
            ...
            for xx in curscaiji:
                ...
                #list_url.append(xxx1aiji)
                scan_cms(xxx1aiji)
                # if __name__ == '__main__':
                #     p = multiprocessing.dummy(target=scan_cms, args=(xxx1aiji,)).start()
                time.sleep(random.randint(1,5))
        except:
            print (unicode('数据库没有网址啊，要不你删除数据库然后重来？','utf-8'))
            time.sleep(150)
if __name__ == '__main__':
    lock =threading.Lock()
    for i in range(int(thread_s)):
        t1 = threading.Thread(target=zairu, args=(), name='CMS Scan Thread -' + str(i)).start()
    t = threading.Thread(target=get_result).start()
