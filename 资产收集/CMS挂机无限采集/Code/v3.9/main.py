...
def scan_waf(uul):
    urls = uul+'/manage.mdb and 1=1 union select user from admin' if uul.startswith('http') else 'http://' + uul + '/manage.mdb and 1=1 union select user from manage'
...
if __name__ == '__main__':
    ...
    ''')
    time.sleep(8)
    New_start = input(('待需验证URL文本拖拽进来:'))
    levels = int(input(('设置检测等级(1/2/3):')))
    threads = int(input(('设置检测线程数(1-1024):')))
    ...
    with ThreadPoolExecutor(threads) as p:  # 类似打开文件,可省去.shutdown()
        future_tasks = [p.submit(scan, name, url, levels,waf) for url in list_]
