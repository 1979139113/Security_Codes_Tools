...
def get_url_from_content(url):
    ...
    mid_result = list(set(result_list))
    #for u in result_list:
        #if 'www.' in u:
        #if 'www.' in u and 'gov.cn' not in u and 'edu.cn' not in u:
    ...
    for u in mid_result:
        try:
            ...
            if r.status_code == 200:
                url_info = {'url':u,'title':get_url_title(u),'power':r.headers.get('X-Powered-By'),'server':r.headers.get('Server')}
                result_set.append(url_info)
        except:
            pass
    return result_set
...
def test_connect_database(user, passwd, host, port, Dbname):
    try:  # line:175
        coon_svn = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                   charset='utf8')  # line:176
...
def scan_waf(uul):
    urls = uul + '/manage.mdb/admin.php?id=2%27 and 1=1 union select user from admin' if uul.startswith(
        'http') else 'http://' + uul + '/manage.mdb/admin.php?id=2%27 and 1=1 union select user from manage'
    print('[ {} ] 检测WAF:{}'.format(datetime.datetime.now(), uul))
    try:
...
def scan_url_cms(url, level, waf):
    if waf == 1:
        wafs = scan_waf(url)
        if wafs != None:
            print('[ {} ] 发现WAF拦截,自动过滤:{}|{}'.format(datetime.datetime.now(), url, wafs))
    ...
    headers = {
        ...
        'Cache-Control': 'max-age=0',
        'Accept-Charset': 'GBK,utf-8;q=0.7,*;q=0.3'}
    print('[ {} ] 检测当前网址:{}'.format(datetime.datetime.now(), url))
        ...
        try:
            ...
            for v in robots:
                if v in url_content:
                    ...
                    return (url + '|对应CMS|' + str(v) + '|robots.txt文件发现关键词|' + v), v, title
    ...
    try:
        ...
        # print('[ {} ] '.format(datetime.datetime.now() + r.url)
        encoding = requests.utils.get_encodings_from_content(r.text)[0]
        url_content = r.content.decode(encoding, 'replace')
        ...
        if level == 1 or level == 2 or level == 3:
            ...
            for k, v in body.items():
                if k in url_content:
                    ...
                    return (url + '|对应CMS|' + str(v) + '|网页发现关键词|' + k), v, title
            for k, v in head.items():
                if k in url_headers:
                    ...
                    return (url + '|对应CMS|' + str(v) + '|请求头发现关键词|' + k), v, title
    except:
    ...
    if level == 2 or level == 3:
        ...
        for x in body_rule:
            ...
            cms_md5 = x.split('|', 3)[2]
            try:
                #        print('[ {} ] '.format(datetime.datetime.now() + url + cms_prefix)
                ...
                if r.status_code == 200:
                    ...
                    if cms_md5 in url_content:
                        ...
                        return (url + '|对应CMS|' + str(
                            cms_name) + '|网址内容发现关键词|' + r.url + '|' + cms_md5), cms_name, title
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if cms_md5 == rmd5:
                        ...
                        return (url + '|对应CMS|' + str(
                            cms_name) + '|网站文件MD5一致|' + r.url + '|' + cms_md5), cms_name, title
...
def send_url_database(level, waf, check_cms,sem):
    ...
    while 1:
        ...
        if urls:
            for u in urls:
                with connect_mysql() as cursor:
                    sqls = 'insert into url_index(url,title,power,server) values ("{}","{}","{}","{}")'.format(u.get('url'),u.get('title'),u.get('power'),u.get('server'))
                    cursor.execute(sqls)
...
if __name__ == '__main__':
    ...
    for i in range(thread_s):
        p.submit(send_url_database, level, waf, check_cms,sem)
