...
thread_url = cfg.get("Config", "thread_url")
thread_cms = cfg.get("Config", "thread_cms")
...
create_sql = [
    ...
    'use ' + Dbname,
    'create table if not EXISTS url_index(id int primary key auto_increment,url varchar(80),urlget varchar(1),datetime varchar(80))charset=utf8,engine=MyISAM;',
    'create table if not EXISTS cms_index(id int primary key auto_increment,url varchar(80),urlget varchar(1),datetime varchar(80))charset=utf8,engine=MyISAM;',
    'create table if not EXISTS url_cms(id int primary key auto_increment,url varchar(80),urlway varchar(100),cmstype varchar(100),urltitle varchar(100),datetime varchar(80))charset=utf8,engine=InnoDB;',
    'create index getid on url_index (urlget);',
    'create index getidc on cms_index (urlget);',
    'create unique index urlique on url_index (url);',
    'create unique index urliquec on cms_index (url);',
    'create index allid on url_cms (url,cmstype,urltitle);'
...
print (unicode('URL采集线程数 : ', 'utf-8')) + str(thread_url)
print (unicode('CMS识别线程数 : ', 'utf-8')) + str(thread_cms)
...
print (unicode(' *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*', 'utf-8'))
time.sleep(0.3)
print (unicode(' *-*-   [+] 首次使用输入 1    (自动安装数据库,输入采集网址)', 'utf-8'))  # line:185
time.sleep(0.5)
print ('\n')  # line:186
...
def scan_baidu(keyword):
    ...
    for xx_0 in list_001:
        ...
        try:
            ...
            sql = "INSERT INTO url_index(url,urlget,datetime) VALUES ('%s','%s','%s')" % (str(xx), '0', str(timenow))
            sql2 = "INSERT INTO cms_index(url,urlget,datetime) VALUES ('%s','%s','%s')" % (str(xx), '0', str(timenow))
            cur.execute(sql)
            coon.commit()
            cur.execute(sql2)
            coon.commit()
...
def wuxiancaiji(url):
    ...
    try:
        ...
        headers = {'User-Agent': UA}
        r = requests.get(url=url, headers=headers, timeout=timeout)
        print str(threading.current_thread().name) + (unicode('    网址无限采集:      ', 'utf-8')) + r.url + '      ' + str(
            r.status_code)
    ...
    for xxx in result_list:
        if 'www.' in xxx:
            try:
                r = requests.head(url=xxx, headers=headers, timeout=timeout)
                print str(threading.current_thread().name) + (unicode('    网址友链采集:      ', 'utf-8')) + r.url + '      ' + str(
                    r.status_code)
                if r.status_code == 200:
                    try:
                        cooncaiji1 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                     charset='utf8')
                        curcaiji1 = cooncaiji1.cursor()
                        timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        sqlcaiji1_1 = "INSERT INTO url_index(url,urlget,datetime) VALUES ('%s','%s','%s')" % (
                        str(xxx), '0', str(timenow))
                        sqlcaiji1_2 = "INSERT INTO cms_index(url,urlget,datetime) VALUES ('%s','%s','%s')" % (
                        str(xxx), '0', str(timenow))
                        curcaiji1.execute(sqlcaiji1_1)
                        cooncaiji1.commit()
                        curcaiji1.execute(sqlcaiji1_2)
                        cooncaiji1.commit()
                        curcaiji1.close()
                        cooncaiji1.close()
                    except Exception, e:
...
print ('\n')
if New_start == '1':
    print (unicode('批量导入网址输入 0 (网址放在当前目录下的url.txt)   自动采集关键词输入 1', 'utf-8'))  # line:185
    time.sleep(3)
    ...
    if setstart == 1:
        keywords = urllib.quote(
            str(raw_input(unicode('输入关键词,采集网址。(例子:inurl:php/浪子帅逼/AT2@32D^%$):', 'utf-8').encode('gbk'))))
    ...
    if setstart == 0:
        try:
            ...
            print unicode('网址加载成功，一共 %s 个网址', 'utf-8') % str(len(urllist))
            time.sleep(2)
            for _ in urllist:
                wuxiancaiji(_)
        except:
            print unicode('目录下没有找到URL.txt', 'utf-8')
            time.sleep(600)
    if setstart != 0 and setstart != 1:
        print unicode('输入的什么鬼呢，干李凉', 'utf-8')
        time.sleep(3)
        print unicode('ヾ(￣▽￣)Bye~Bye~', 'utf-8')
...
def scan_cms(url):
    ...
    try:
        ...
        print (
        str(threading.current_thread().name) + unicode('    CMS类型识别:    ', 'utf-8') + '  ' + req3.url + '    ' + str(
            req3.status_code))
        for key, valu in body.iteritems():
            if key in req3.content:
                try:
                    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    coon_cms_1 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                 charset='utf8')
                    cur_cms_1 = coon_cms_1.cursor()
                    sql_cms_1 = "INSERT INTO url_cms (url,urlway,cmstype,urltitle,datetime) VALUES (%s,%s,%s,%s,%s)"
    ...
    try:
        ...
        print (str(threading.current_thread().name) + unicode('    CMS类型识别:    ',
                                                              'utf-8') + '  ' + req2.url + '    ' + str(
            req2.status_code))
        for x_x in robots:
            if x_x in req2.content:
                try:
                    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    coon_cms_2 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                 charset='utf8')
                    cur_cms_2 = coon_cms_2.cursor()
                    sql_cms_2 = "INSERT INTO url_cms (url,urlway,cmstype,urltitle,datetime) VALUES (%s,%s,%s,%s,%s)"
                ...
                try:
                    ...
                    if x['md5'] == '':
                        ...
                        if d > 0:
                            try:
                                timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                coon_cms_3 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname,
                                                             port=port, charset='utf8')
                                cur_cms_3 = coon_cms_3.cursor()
                                sql_cms_3 = "INSERT INTO url_cms (url,urlway,cmstype,urltitle,datetime) VALUES (%s,%s,%s,%s,%s)"
                                cur_cms_3.execute(sql_cms_3, (
                                    str(url), str(urlway), str(cmstype), str(cms_title), str(timenow)))
                                coon_cms_3.commit()
                                cur_cms_3.close()
                                coon_cms_3.close()
                                return ''
    ...
    if rrsa != None:
        ...
        for s in st:
            ...
            try:
                ...
                print (str(threading.current_thread().name) + unicode('    STRUTS 2 类型识别:    ',
                                                                      'utf-8') + '  ' + req1.url + '    ' + str(
                    req1.status_code))
                if req1.status_code == 200:
                    ...
                    #print 'difflib:' + str(didx) + '        ' + str(req1_2.url)
                    if '.action' in req1_2.content and didx < 4321:
                        try:
                            timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                            coon_cms_3 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                         charset='utf8')
                            cur_cms_3 = coon_cms_3.cursor()
                            sql_cms_3 = "INSERT INTO url_cms (url,urlway,cmstype,urltitle,datetime) VALUES (%s,%s,%s,%s,%s)"
                            cur_cms_3.execute(sql_cms_3,
                                              (str(urlst), str(s), str('Apache Struts 2'), str(cms_title), str(timenow)))
                            coon_cms_3.commit()
                            cur_cms_3.close()
                            coon_cms_3.close()
                            return ''
    ...
    for cmsxx in cms_rule:
        ...
        try:
            ...
            if req1.status_code == 200:
                ...
                rmd5 = md5.hexdigest()
                #print req1_2.url + '    ' + str(rmd5) + ':' + str(cmsmd5)
                if rmd5 == cmsmd5:
                    #print '*******' + cmsname + '*******************'
                    try:
                        timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        ...
                        sql_cms_3 = "INSERT INTO url_cms (url,urlway,cmstype,urltitle,datetime) VALUES (%s,%s,%s,%s,%s)"
                        cur_cms_3.execute(sql_cms_3,
                                          (str(url), str(cmshouzhui), str(cmsname), str(cms_title), str(timenow)))
                        coon_cms_3.commit()
                        cur_cms_3.close()
                        coon_cms_3.close()
                        return ''
...
def zairu():
    while 1:
        try:
            ...
            time.sleep(random.randint(1, 5))
            lock.acquire()
            cooncaiji2 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                         charset='utf8')
            ...
            print (unicode('网址数据库没有网址啊，要不你删除数据库然后重来？', 'utf-8'))
            time.sleep(150)
    ...
    while 1:
        try:
            ...
            cooncaiji2 = pymysql.connect(user=user, passwd=passwd, host=host, db=Dbname, port=port,
                                                         charset='utf8')
            curcaiji2 = cooncaiji2.cursor()
            sqlcaiji2 = "select url from cms_index where urlget=0 limit " + str(0) + ",1"  # 10表示载入10个网址
            sqlcaiji2_1 = "update cms_index set urlget='1' where urlget = 0 limit 1"
            curcaiji2.execute(sqlcaiji2)
            cooncaiji2.commit()
            curscaiji = curcaiji2.fetchone()
            curcaiji2.execute(sqlcaiji2_1)
            ...
            print (unicode('CMS数据库没有网址啊，要不你删除数据库然后重来？', 'utf-8'))
            time.sleep(150)
...
if __name__ == '__main__':
    lock = threading.Lock()
    if int(thread_url)>0:
        for i in range(int(thread_url)):
            t1 = threading.Thread(target=zairu, args=(), name='URL Scan Thread - ' + str(i)).start()
    if int(thread_cms)>0:
        for i in range(int(thread_cms)):
                t2 = threading.Thread(target=zairucms, args=(), name='CMS Scan Thread - ' + str(i)).start()
